...
def autoencoder(im_size, filters, act='relu', init='glorot_uniform'):
    ...
    x = layers.Conv2D(filters[4], kernel, activation=act,
                      padding='same', name='hidden_layer')(x)
    ...
    ae.summary()
    encoder = Model(inputs=ae.input, outputs=ae.get_layer('hidden_layer').output)
    return ae, encoder
...
class DEC(object):
    ...
    def pretrain(self, x, y=None, optimizer='adam', epochs=200, batch_size=256, save_dir='results/temp'):
        ...
        if y is not None:
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    print('+ here: ' + 'encoder_%d' %
                          (int(len(self.autoencoder.layers)) - 1))
                    features_model.summary()

                    features = features_model.predict(self.x)
            ...
            cb.append(PrintACC(x, y, self.autoencoder))
